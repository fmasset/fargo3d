...
void AdvanceSystemRK5 (real dt) {
  ...
  if ((PhysicalTime < RELEASEDATE) || (fabs(FORCEOMEGA) > 1e-15*sqrt(G*MSTAR/(R0*R0*R0))))
    startindex = 1;

  for (i = startindex; i < Sys->nb; i++) {
    Sys->x[i]  = q1[i];
    Sys->y[i]  = q1[i+n];
    Sys->z[i]  = q1[i+2*n];
    Sys->vx[i] = q1[i+3*n];
  ...
  if (startindex) { //We hereafter assume the planet to be in the plane.
    x = Sys->x[0];
    y = Sys->y[0];
    r = sqrt(x*x+y*y);
    theta = atan2(y,x);
    ...
    } else if (FORCEOMEGA > 0.0) {
      omega = FORCEOMEGA;
      rdot = 0.0;
    }
    new_r = r + rdot*dt;
    denom = r-new_r;
    if (fabs(denom) > 1e-15*R0) {
